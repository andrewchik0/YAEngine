...
def load_manifest(shot_dir):
    ...
    if not os.path.isfile(path):
        sys.exit("no manifest.json in %s" % shot_dir)
...
def find_target(manifest, name=None):
    ...
    if not targets:
        sys.exit("manifest lists no targets")
    if name is None:
        return targets[0]
    for target in targets:
        if target["name"] == name:
            return target
    sys.exit("no target '%s'; have %s" % (name, ", ".join(t["name"] for t in targets)))
...
def load_target(shot_dir, target, filename=None):
    ...
    if raw.size != expected:
        sys.exit("%s holds %d elements, manifest implies %d" % (path, raw.size, expected))
...
def crop(image, rect):
    ...
    if x < 0 or y < 0 or x + w > width or y + h > height or w <= 0 or h <= 0:
        sys.exit("rect %s does not fit in %dx%d" % (rect, width, height))
...
def print_stats(target, image, label):
    ...
    print("%s  %s  %s  %s" % (
        ...
        target["colorSpace"]))
    print("  pixels=%d  NaN=%d  Inf=%d" % (image.shape[0] * image.shape[1], nan, inf))
    if target["format"]["name"] == "D32_SFLOAT":
        print("  reversed-Z: 0 is the far plane, 1 is the near plane")

    header = "  %-6s %10s %10s %10s %10s %10s %10s %10s"
    print(header % ("chan", "min", "p1", "p50", "mean", "p99", "p99.9", "max"))
    for index, name in enumerate(names):
        ...
        print(header % (name, "%.4g" % column.min(), "%.4g" % p1, "%.4g" % p50,
                        "%.4g" % column.mean(), "%.4g" % p99, "%.4g" % p999,
                        "%.4g" % column.max()))

    peak = np.nanmax(np.where(np.isfinite(image), image, np.nan), axis=2)
    total = peak.size
    print("  fraction over 1/10/100: %.5f %.5f %.5f" % (
        np.count_nonzero(peak > 1.0) / total,
        np.count_nonzero(peak > 10.0) / total,
        np.count_nonzero(peak > 100.0) / total))
...
def cmd_summary(args):
    ...
    if os.path.isfile(session_path):
        ...
        print("session %s  %s  %s" % (session["status"], session["startedUtc"],
                                      session["gpu"]["name"]))
        print("scene %s" % session["scene"])
        shot_dirs = [os.path.join(args.capture_dir, s["dir"]) for s in session["shots"]]
    else:
        shot_dirs = sorted(
            os.path.join(args.capture_dir, name)
            for name in os.listdir(args.capture_dir)
            if os.path.isdir(os.path.join(args.capture_dir, name)))

    print("%-14s %-12s %-14s %8s %-14s %s" % (
...
def cmd_stats(args):
    manifest = load_manifest(args.shot_dir)
    target = find_target(manifest, args.target)
    image = load_target(args.shot_dir, target)
    rect = parse_rect(args.rect) if args.rect else None
    label = target["name"] if rect is None else "%s rect=%s" % (target["name"], args.rect)
    print_stats(target, crop(image, rect), label)
...
def cmd_px(args):
    ...
    if not (0 <= args.x < width and 0 <= args.y < height):
        sys.exit("pixel (%d, %d) is outside %dx%d" % (args.x, args.y, width, height))

    names = channel_names(target, image.shape[2])
    values = image[args.y, args.x]
    print("%s (%d, %d) %s" % (target["name"], args.x, args.y, target["format"]["name"]))
    for name, value in zip(names, values):
        print("  %-6s %.9g" % (name, value))
...
def cmd_rect(args):
    ...
    print_stats(target, region, "%s rect=%s" % (target["name"], args.rect))

    if args.dump:
        count = region.shape[0] * region.shape[1]
        if count > 64:
            sys.exit("--dump refuses %d pixels; keep the rect at 64 or fewer" % count)
        ...
        for row in range(region.shape[0]):
            for column in range(region.shape[1]):
                print("  (%d,%d) %s" % (rect[0] + column, rect[1] + row,
                                        format_row(region[row, column])))
...
def cmd_preview(args):
    ...
    target = find_target(manifest, args.target)
    image = load_target(args.shot_dir, target)
    mapped = tone_map(image, args.map, args.exposure)

    out = args.out or os.path.join(args.shot_dir, "%s.%s.preview.png" % (target["name"], args.map))
    Image.fromarray((np.clip(mapped, 0.0, 1.0) * 255.0 + 0.5).astype(np.uint8)).save(out)
    print("%s -> %s  (%s, exposure %g)" % (target["name"], out, args.map, args.exposure))


def cmd_diff(args):
    left = load_manifest(args.shot_a)
    right = load_manifest(args.shot_b)

    print("settings that differ (%s vs %s):" % (left["shot"]["name"], right["shot"]["name"]))
    differing = 0
    for key in sorted(set(left["settings"]) | set(right["settings"])):
        a = left["settings"].get(key)
        b = right["settings"].get(key)
        if a != b:
            print("  %-22s %s  ->  %s" % (key, a, b))
            differing += 1
    if differing == 0:
        print("  (none)")

    target_a = find_target(left, args.target)
    target_b = find_target(right, args.target or target_a["name"])
    if target_a["extent"] != target_b["extent"]:
        print("extents differ (%s vs %s), skipping the image diff"
              % (target_a["extent"], target_b["extent"]))
        return

    image_a = np.nan_to_num(load_target(args.shot_a, target_a))
    image_b = np.nan_to_num(load_target(args.shot_b, target_b))
    delta = image_b - image_a
    names = channel_names(target_a, delta.shape[2])

    print("image diff on '%s':" % target_a["name"])
    for index, name in enumerate(names):
        column = delta[:, :, index]
        print("  %-6s mean=%+.5g  mean|d|=%.5g  max|d|=%.5g"
              % (name, column.mean(), np.abs(column).mean(), np.abs(column).max()))

    magnitude = np.abs(delta[:, :, :3]).mean(axis=2) if delta.shape[2] >= 3 \
        else np.abs(delta[:, :, 0])
    print("  where the difference lives:")
    for line in ascii_map(magnitude):
        print(line)
...
def cmd_temporal(args):
    ...
    if "pattern" not in files:
        sys.exit("'%s' holds a single frame; capture it with frames=N for a temporal run"
                 % target["name"])

    frames = []
    for index in range(manifest["timing"]["framesCaptured"]):
        path = files["pattern"] % index
        if not os.path.isfile(os.path.join(args.shot_dir, path)):
            break
        frames.append(np.nan_to_num(load_target(args.shot_dir, target, path)))

    if len(frames) < 3:
        sys.exit("only %d frames on disk, need at least 3" % len(frames))
    ...
    print("%s  %d frames  mean_lum=%.4f" % (target["name"], len(frames), base))
    ...
    print("  pixels unstable = %.2f%%" % (100 * float(np.mean(deviation > 0.02 * base))))
    print("  instability map:")
    for line in ascii_map(deviation):
        print(line)
...
def main():
    ...
    preview.add_argument("--map", choices=["linear", "log", "auto", "falsecolor"], default="auto")
