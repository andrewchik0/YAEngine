...
void main()
{
  ivec2 screenSpaceUV = ivec2(uv * vec2(u_Data.screenWidth, u_Data.screenHeight));
  ...
  if (historyValid)
  {
    ...
    float blendFactor = mix(EMA_IIR_INVERSE_CUTOFF_FREQUENCY, 0.0, clamp(speed / 16.0, 0.0, 1.0));
