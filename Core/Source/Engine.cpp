...
namespace YAEngine
{
      ...
      m_InputSystem(m_Window)
  {
    ...
    m_Registry.Register<SequencePlayer>(&m_SequencePlayer);
  ...
  void Engine::Run()
  {
    ...
    while (m_Window.IsOpen())
    {
      ...
      {
        // Real delta on purpose: a sequence is meant to be sampled at whatever frame rate
        // the machine reaches, not at the fixed step reel playback pins frameDt to. Runs
        // before the systems so TransformSystem picks the pose up in this same frame, and
        // before the layers, so gameplay code reads this frame's timeline time.
        YA_PROFILE_CPU("Sequence");
        m_SequencePlayer.Update(m_Scene, m_Timer.GetDeltaTime());
      }
