...
namespace YAEngine
{
  void RoadSystem::Update(entt::registry& registry, double dt)
  {
    ...
    auto view = registry.view<RoadComponent, RoadDirty>();
    for (auto entity : view)
    {
      auto& road = registry.get<RoadComponent>(entity);
      ...
      ProcMesh procMesh = RoadMeshGenerator::Generate(road);

      if (procMesh.vertices.empty())
      {
        registry.remove<RoadDirty>(entity);
        continue;
      }
