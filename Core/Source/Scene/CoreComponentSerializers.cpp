  ...
  void RegisterCoreComponentSerializers(ComponentRegistry& registry, AssetManager& assets)
  {
    ...
    registry.Register<LightComponent>("light",
      [](const entt::registry& reg, entt::entity e) -> YAML::Node {
        ...
        if (l.type != LightType::Directional && l.sourceRadius != 0.0f)
          n["sourceRadius"] = l.sourceRadius;
        if (l.type == LightType::Directional && l.angularDiameter != 0.53f)
          n["angularDiameter"] = l.angularDiameter;
        if (l.rasterOnly)
          n["rasterOnly"] = true;
        return n;
      },
      [](entt::registry& reg, entt::entity e, const YAML::Node& n) {
        ...
        if (n["sourceRadius"]) l.sourceRadius = n["sourceRadius"].as<float>();
        if (n["angularDiameter"]) l.angularDiameter = n["angularDiameter"].as<float>();
        if (n["rasterOnly"]) l.rasterOnly = n["rasterOnly"].as<bool>();
    ...
    registry.Register<ColliderComponent>("collider",
      [](const entt::registry& reg, entt::entity e) -> YAML::Node {
        ...
        YAML::Node n;
        n["shape"] = "aabb";
        if (c.localOffset != glm::vec3(0.0f))
          n["localOffset"] = SerializeVec3(c.localOffset);
        if (c.halfExtents != glm::vec3(0.5f))
          n["halfExtents"] = SerializeVec3(c.halfExtents);
        if (!c.isStatic)
      ...
      [](entt::registry& reg, entt::entity e, const YAML::Node& n) {
        ...
        {
          auto s = n["shape"].as<std::string>();
          if (s == "aabb") c.shape = ColliderShape::AABB;
        }
        if (n["localOffset"]) c.localOffset = DeserializeVec3(n["localOffset"]);
        if (n["halfExtents"]) c.halfExtents = DeserializeVec3(n["halfExtents"]);
        if (n["isStatic"]) c.isStatic = n["isStatic"].as<bool>();
        if (n["layer"]) c.layer = n["layer"].as<uint32_t>();
        if (n["mask"]) c.mask = n["mask"].as<uint32_t>();
        reg.emplace_or_replace<ColliderComponent>(e, c);
      }
    );
    ...
    registry.Register<InstancedColliderComponent>("instancedCollider",
      ...
      [](entt::registry& reg, entt::entity e, const YAML::Node& n) {
        ...
        if (n["layer"]) c.layer = n["layer"].as<uint32_t>();
        if (n["mask"]) c.mask = n["mask"].as<uint32_t>();
        reg.emplace_or_replace<InstancedColliderComponent>(e, c);
      }
    );
