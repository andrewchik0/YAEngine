...
namespace YAEngine
{
  ...
  void VulkanMaterial::Init()
  {
    ...
    for (size_t i = 0; i < s_MaxFramesInFlight; i++)
    {
      ...
      m_DescriptorSets[i].BindUniformBuffer(0, m_UniformBuffers[i].Get(), sizeof(__PerMaterialData));
      for (size_t j = 1; j < 8; j++)
        m_DescriptorSets[i].BindCombinedImageSampler((uint32_t)j, s_NoneTexture.GetView(), s_NoneTexture.GetSampler());
    }
  }
  ...
  void VulkanMaterial::Bind(Application* app, Material& material, uint32_t currentFrame)
  {
    ...
    if (app->GetAssetManager().Textures().Has(material.baseColorTexture))
    {
      ...
      m_DescriptorSets[currentFrame].BindCombinedImageSampler(1, base.GetView(), base.GetSampler());
      textureMask |= (1 << 0);
    }
    if (app->GetAssetManager().Textures().Has(material.metallicTexture))
    {
      ...
      m_DescriptorSets[currentFrame].BindCombinedImageSampler(2, metallic.GetView(), metallic.GetSampler());
      textureMask |= (1 << 1);
    }
    if (app->GetAssetManager().Textures().Has(material.roughnessTexture))
    {
      ...
      m_DescriptorSets[currentFrame].BindCombinedImageSampler(3, roughness.GetView(), roughness.GetSampler());
      textureMask |= (1 << 2);
    }
    if (app->GetAssetManager().Textures().Has(material.specularTexture))
    {
      ...
      m_DescriptorSets[currentFrame].BindCombinedImageSampler(4, specular.GetView(), specular.GetSampler());
      textureMask |= (1 << 3);
    }
    if (app->GetAssetManager().Textures().Has(material.emissiveTexture))
    {
      ...
      m_DescriptorSets[currentFrame].BindCombinedImageSampler(5, emissive.GetView(), emissive.GetSampler());
      textureMask |= (1 << 4);
    }
    if (app->GetAssetManager().Textures().Has(material.normalTexture))
    {
      ...
      m_DescriptorSets[currentFrame].BindCombinedImageSampler(6, normal.GetView(), normal.GetSampler());
      textureMask |= (1 << 5);
    }
    ...
    m_DescriptorSets[currentFrame].BindUniformBuffer(0, m_UniformBuffers[currentFrame].Get(), sizeof(__PerMaterialData));
  }
}
